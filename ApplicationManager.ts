export default class ApplicationManager { // TEST
    appName: string; // Deklariert eine öffentliche Eigenschaft 'appName' vom Typ String.
    loginForm: HTMLFormElement;// Deklariert eine öffentliche Eigenschaft 'loginForm' vom Typ HTMLFormElement.
    signupForm: HTMLElement;
    linkToSignup: HTMLElement;
    linkToLogin: HTMLElement;
    startPage: HTMLElement;
    logoutLink: HTMLAnchorElement;
    registeredUsers = new Map<string, string>();
    impressumContainer: HTMLElement;
    ...
    constructor(name: string) {
        this.appName = name;
        this.loginForm = document.getElementById('FormLogin') as HTMLFormElement;
        this.signupForm = document.getElementById('FormSignup') as HTMLElement;
        this.linkToSignup = document.getElementById('LinkShowSignupDialog') as HTMLElement;
        this.linkToLogin = document.getElementById('LinkShowLoginDialog') as HTMLElement;
        this.startPage = document.getElementById('StartPage') as HTMLElement;
        this.logoutLink = document.getElementById('LinkLogout') as HTMLAnchorElement;
        this.impressumContainer = document.getElementById('ImpressumContainer') as HTMLElement;
        ...
        if (!this.loginForm) console.error('ERROR: Login-Formular nicht gefunden.');
        if (!this.signupForm) console.error('ERROR: Registrierungs-Formular nicht gefunden.');
        if (!this.linkToSignup) console.error('ERROR: Link zu Signup nicht gefunden.');
        if (!this.linkToLogin) console.error('ERROR: Link zu Login nicht gefunden.');
        if (!this.startPage) console.error('ERROR: Startseiten-Element nicht gefunden.');
        if (!this.logoutLink) console.warn('WARNUNG: Logout-Link nicht gefunden.');
        if (!this.impressumContainer) console.error('ERROR: Impressum-Container nicht gefunden.');
    ...
    handleLogin() {
        ...
        const loginUsernameInput = document.getElementById('FormLoginUsername') as HTMLInputElement;
        const loginPasswordInput = document.getElementById('FormLoginPassword') as HTMLInputElement;
        ...
        if (loginUsernameInput && loginPasswordInput) {
            ...
            if (this.registeredUsers.has(username) && this.registeredUsers.get(username) === password) {
                ...
                if (this.startPage) this.startPage.style.display = 'block';
                if (userManagementMenuItem) userManagementMenuItem.style.display = 'block';
                if (logoutButton) logoutButton.style.display = 'block';
                if (this.loginForm) this.loginForm.style.display = 'none';
                if (this.signupForm) this.signupForm.style.display = 'none';
                if (this.impressumContainer) this.impressumContainer.style.display = 'none'; // Ausblenden des Impressum-Containers
            } else {
    ...
    handleSignup() {
        ...
        const signupUsernameInput = document.getElementById('FormSignupUsername') as HTMLInputElement;
        const signupPasswordInput = document.getElementById('FormSignupPassword') as HTMLInputElement;
    ...
    handleLogout(): void {
        ...
        if (userManagementMenuItem) userManagementMenuItem.style.display = 'none';
        if (this.impressumContainer) this.impressumContainer.style.display = 'none';
